...
def _load_bold_font(size_px: int):
    """Find a bold sans-serif TTF across Mac/Linux. Fall back to PIL default.
    Return type intentionally untyped — Pillow returns ImageFont.ImageFont or
    ImageFont.FreeTypeFont depending on which loader succeeds; both are
    duck-compatible with the methods we call on the result.
    """
    candidates = [
        # macOS
...
def _make_icon(strike: bool = False) -> Image.Image:
    """Render a small monochrome "em" wordmark.

    Monochrome (near-black) so macOS dark/light menubar adaptation looks
    right and the icon stays native-feeling. Strike-through marks "stopped";
    proxy on/off is conveyed in the menu, not the icon.
    """
    size = 22
    img = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(img)
    font = _load_bold_font(11)

    # Near-black with full alpha. macOS will tint this appropriately in dark mode
    # when the icon ships as a template; pystray doesn't expose template mode
    # uniformly, but near-black reads well on both light and dark menubars.
    color = (0, 0, 0)
    text = "em"
    ...
    if strike:
        d.line((2, size - 3, size - 3, 2), fill=(*color, 255), width=2)
    return img


ICON_RUNNING_ON = _make_icon()                  # "em"
ICON_RUNNING_OFF = _make_icon()                 # same — proxy state shown in menu
ICON_STOPPED = _make_icon(strike=True)          # "em" with strike = service down
