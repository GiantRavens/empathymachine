...
async fn handle_request(req: Request<Body>, state: Arc<AppState>) -> Result<Response<Body>, Infallible> {
    let response = match (req.method(), req.uri().path()) {
        ...
        (&Method::GET, "/api/lsrules") | (&Method::HEAD, "/api/lsrules") => {
            lsrules_handler(state).await
        }
        _ => Response::builder()
...
// ─── /api/lsrules — Little Snitch Subscribed Rule Group publisher ────────
//
// Generates a `.lsrules`-compatible JSON document from current policy
// (config.yaml's network_policy plus optionally custom rule lists). The
// captain configures LS once: Rules → Subscribed Rule Groups → Add →
// URL: http://127.0.0.1:8081/api/lsrules. LS pulls periodically; every
// policy change made via `mm policy` becomes an LS rule update.
//
// Returns JSON with content-type application/json so curl, browsers, and
// LS's own URL fetcher all handle it correctly.
...
fn build_lsrules(config: &Config) -> LsRules {
    let mut rules = Vec::new();

    if config.network_policy.block_quic {
        rules.push(LsRule {
            ...
            notes: "Block QUIC (HTTP/3 over UDP/443) so clients fall back \
                    to HTTPS/TCP and EmpathyMachine retains full inspection. \
                    Decided via policy.yaml.network_policy.block_quic — see \
                    `mm policy why block_quic` for full reasoning trace."
                .to_string(),
        });
    }

    LsRules {
        name: "EmpathyMachine Policy".to_string(),
        description: format!(
            "Auto-generated by EmpathyMachine. Source of truth: policy.yaml. \
             Rule count: {}. To subscribe in Little Snitch: Rules → Subscribed \
             Rule Groups → Add → URL of this endpoint.",
            rules.len()
        ),
        rules,
    }
...
async fn lsrules_handler(state: Arc<AppState>) -> Response<Body> {
    let lsrules = build_lsrules(&state.config);
    match serde_json::to_vec_pretty(&lsrules) {
        Ok(bytes) => Response::builder()
            .status(StatusCode::OK)
            .header("content-type", "application/json; charset=utf-8")
            // Hint to LS + browsers that this is a subscription rule file
            .header("content-disposition",
                    "inline; filename=\"empathymachine.lsrules\"")
            .body(Body::from(bytes))
            .unwrap(),
        Err(err) => {
            tracing::error!(error = %err, "failed to serialize lsrules");
            Response::builder()
